...
socket.on('status_texts', function(data) {
  ...
  if (data.texts && data.texts.length > 0) {
    ...
    list.innerHTML = '<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><p>No status texts added.</p></div>';
  }
});
...
function buildRichPresenceEditor() {
  const html = '<div class="glass-card">' +
    ...
    '<div style="flex:1; min-width:280px;" id="rpc-form">' +
  ...
  setTimeout(() => {
    ...
    if (form) {
      form.querySelectorAll('input, select').forEach(el => {
        el.addEventListener('input', updateRichPresencePreview);
        el.addEventListener('change', updateRichPresencePreview);
      });
      loadFormDraft('rpcDraft', 'rpc-form');
